...
def load_aviation_network(file_path: str) -> Optional[nx.Graph]:
    print(f"Loading aviation network from: {os.path.basename(file_path)}")
    ...
    try:
        ...
        print(f"   Loaded {G.number_of_nodes()} airports, {G.number_of_edges()} routes")
...
def build_invasive_network(
        data_file: str,
        target_ratio: float = 0.05,
        max_peripheral_degree: int = 5,
        seed: int = 42
) -> Tuple[Optional[nx.Graph], Optional[List[int]]]:
    print("=" * 60)
    print("Building Aviation Network for Invasive Species Simulation")
    print("=" * 60)

    print("\n1. Loading aviation network data...")
    if not os.path.exists(data_file):
        print(f"   Error: File not found: {data_file}")
        return None, None

    G = load_aviation_network(data_file)
    if G is None:
        return None, None
    ...
    if not nx.is_connected(G):
        ...
        print(f"   LCC size: {G.number_of_nodes()} airports, {G.number_of_edges()} routes")
    else:
        print(f"   Network is connected ({G.number_of_nodes()} airports)")
    ...
    print(f"\n4. Target count: {n_targets} ({target_ratio:.1%} of {n_total} total airports)")

    print(f"\n5. Identifying peripheral airports (degree ≤ {max_peripheral_degree})...")
    degrees = dict(G.degree())

    peripheral_nodes = [n for n, d in degrees.items() if d <= max_peripheral_degree]
    print(f"   Found {len(peripheral_nodes)} peripheral airports "
    ...
    if len(peripheral_nodes) < n_targets:
        ...
        print(f"   Now have {len(peripheral_nodes)} candidate airports")
    ...
    print(f"\n6. Selecting initial invasion entry points from peripheral airports...")
    ...
    print(f"   Selected {len(target_nodes)} initial contaminated shipments")
    print(f"   Average degree of entry airports: {avg_target_degree:.2f}")

    print(f"\n{'=' * 60}")
    print(f"Aviation Network Summary:")
    print(f"  Airports (nodes): {G.number_of_nodes()}")
    print(f"  Routes (edges): {G.number_of_edges()}")
    ...
    hub_nodes = [n for n, d in degrees.items() if d > 50]
    periph_count = len([n for n, d in degrees.items() if d <= max_peripheral_degree])
    print(f"  Peripheral airports (degree ≤ {max_peripheral_degree}): "
          f"{periph_count} ({periph_count / n_total:.1%})")
    print(f"  Hub airports (degree > 50): {len(hub_nodes)} ({len(hub_nodes) / n_total:.1%})")
    print(f"  Initial contaminated shipments: {len(target_nodes)} ({len(target_nodes) / n_total:.2%})")
    print(f"{'=' * 60}")
...
class InvasiveSpeciesSimulator:
    ...
    def _execute_invasion_movement(self):
        if self.move_prob <= 0:
            return
        ...
        for front_node in current_fronts:
            ...
            if valid_neighbors and self.rng.random() < self.move_prob:
                ...
                colonized_this_round.add(front_node)
            else:
                new_front_positions.add(front_node)
                claimed_positions.add(front_node)

        self.colonized_nodes |= colonized_this_round
    ...
    def execute_spread_if_round_complete(self):
        if self.attacks_in_current_round >= self.attack_per_step:
            if not self.is_done():
                self._execute_invasion_movement()
            self.attacks_in_current_round = 0
    ...
    def get_metrics(self) -> Tuple[float, float, float]:
        ...
        invasion_ratio = len(self.colonized_nodes) / max(1, self.original_total_nodes)
        return pc, anc, invasion_ratio


def simulate_invasive_heuristic(G: nx.Graph, target_nodes: List[int], move_prob: float,
                                attack_per_step: int, method_func: Callable,
                                seed: Optional[int] = None,
                                batch_mode: bool = False) -> Tuple[float, float, float, bool]:
    simulator = InvasiveSpeciesSimulator(G, target_nodes, move_prob, attack_per_step, seed)
    ...
    pc, anc, invasion_ratio = simulator.get_metrics()
    return pc, anc, invasion_ratio, simulator.is_done()
...
def create_invasive_pyg_data(G: nx.Graph, target_nodes: set, removed_nodes: set,
                             colonized_nodes: set, move_prob: float, attacks_in_round: int,
...
def simulate_invasive_rl(G: nx.Graph, target_nodes: List[int], rl_agent,
...
def _run_td_task(args):
    ...
    pc, anc, invasion, success = simulate_invasive_heuristic(
        G, targets, move_prob, attack_per_step,
        td_method_adaptive, sim_seed, batch_mode=True
    )
    return {'pc': pc, 'anc': anc, 'invasion': invasion,
...
def _run_rl_task(args):
    ...
    if _worker_rl_agent is None:
        return {'pc': None, 'anc': None, 'invasion': None, 'lambda_eff': lambda_eff, 'sim_seed': sim_seed}

    G = _worker_graph.copy()
    targets = list(_worker_targets)

    pc, anc, invasion, success = simulate_invasive_rl(
        G, targets, _worker_rl_agent, move_prob, attack_per_step, sim_seed
    )
    return {'pc': pc, 'anc': anc, 'invasion': invasion, 'lambda_eff': lambda_eff, 'sim_seed': sim_seed}


def run_invasive_experiment(
        G: nx.Graph,
    ...
    print(f"\n{'=' * 70}")
    print(f"🦟 Invasive Species (Aviation): Full 4-Method Comparison")
    print(f"{'=' * 70}")
    print(f"Network: N={G.number_of_nodes()}, E={G.number_of_edges()}")
    print(f"Model: Non-Destructive Random Walk (colonized airports stay in graph)")
    print(f"Initial contaminated shipments: {len(target_nodes)}")
    ...
    results = {m: defaultdict(lambda: {'pc': [], 'anc': [], 'invasion': [], 'success': [], 'seeds': []}) for m in all_methods}
    ...
    with ProcessPoolExecutor(
        ...
        for result in tqdm(executor.map(_run_td_task, td_tasks),
            ...
            results['TD'][result['lambda_eff']]['invasion'].append(result['invasion'])
    ...
    with ProcessPoolExecutor(
        ...
        for result in tqdm(executor.map(_run_tia_task, tia_tasks),
            ...
            results['TIA'][result['lambda_eff']]['invasion'].append(result['invasion'])
    ...
    with ProcessPoolExecutor(
        ...
        for result in tqdm(executor.map(_run_katz_task, katz_tasks),
            ...
            results['Katz'][result['lambda_eff']]['invasion'].append(result['invasion'])
        ...
        with ProcessPoolExecutor(
            ...
            for result in tqdm(executor.map(_run_rl_task, rl_tasks),
                ...
                if result['pc'] is not None:
                    ...
                    results['GNN-RL'][result['lambda_eff']]['invasion'].append(result['invasion'])
...
def save_network_with_targets(
        ...
        target_nodes: List[int],
        save_dir:     str,
        graphml_name: str = 'invasive_network.graphml',
        txt_name:     str = 'invasive_target_nodes.txt',
) -> None:
    ...
    with open(txt_path, 'w') as fh:
        fh.write('# Invasive species (aviation) -- initial contaminated shipment airports\n')
        fh.write(
...
def save_results_to_csv(
    ...
    for method, lam_dict in results.items():
        for lam in sorted(lambda_values):
            ...
            im, is_, ie = _s(inv_vals) if inv_vals else (0., 0., 0.)
            ...
            rows.append({
                ...
                'invasion_mean':   im,  'invasion_std': is_, 'invasion_sem': ie,
    ...
    csv_path = os.path.join(save_dir, f'invasive_results_{ts}.csv')
...
def print_results_summary(results: Dict, lambda_values: List[float]):
    ...
    for lambda_eff in lambda_values:
        print(f"\nλ = {lambda_eff}")
        print(f"{'-' * 90}")
        print(f"{'Method':<15} {'PC (mean±std)':<22} {'ANC (mean±std)':<22} {'Invasion (mean±std)':<22}")
        print(f"{'-' * 90}")
        ...
        for method in results:
            if results[method][lambda_eff]['pc']:
                ...
                inv_mean = np.mean(results[method][lambda_eff]['invasion'])
                inv_std = np.std(results[method][lambda_eff]['invasion'])
                print(
                    f"{method:<15} {pc_mean:.4f}±{pc_std:.4f}          {anc_mean:.4f}±{anc_std:.4f}          {inv_mean:.4f}±{inv_std:.4f}")
...
def plot_results(
    ...
    ax.set_ylabel('PC (Quarantine Cost)', fontsize=12)
    ...
    for method in methods:
        ...
        sems = []
        for lam in lambda_values:
            vals = results[method][lam]['invasion']
            if vals:
                lambdas.append(lam)
                means.append(np.mean(vals))
                sems.append(np.std(vals) / np.sqrt(len(vals)))
    ...
    initial_ratio = n_initial_fronts / n_nodes
    ax.axhline(y=initial_ratio, color='gray', linestyle='--', linewidth=1.5,
               label=f'Initial fronts ({initial_ratio:.2%})')

    ax.set_xlabel('$\\lambda_{eff}$', fontsize=12)
    ax.set_ylabel('Invasion Ratio', fontsize=12)
    ax.set_title('Invasion Footprint (Colonized Airports)', fontsize=13, fontweight='bold')
    ax.legend(loc='best', fontsize=10)
    ax.grid(True, alpha=0.3, linestyle='--')

    target_ratio = n_initial_fronts / n_nodes
    n_methods = len(methods)
    fig.suptitle(
        f'Invasive Species (Aviation): {n_methods}-Method Comparison | '
        f'N={n_nodes}, Shipments={n_initial_fronts} ({target_ratio:.1%})',
    ...
    if save_dir:
        ...
        filename = f'invasive_species_results_{timestamp}.png'
...
class InvasiveSnapshotSimulator(InvasiveSpeciesSimulator):
...
def _run_snapshot_invasive_heuristic(G, target_nodes, move_prob, attack_per_step,
                                      method_func, seed, batch_mode=False):
    sim = InvasiveSnapshotSimulator(G, target_nodes, move_prob, attack_per_step, seed)
    ...
    pc, anc, invasion = sim.get_metrics()
    return pc, anc, invasion, sim.is_done(), sim.get_removed_nodes(), sim.get_final_targets()


def _run_snapshot_invasive_rl(G, target_nodes, rl_agent, move_prob,
...
def save_terminal_snapshot(
    ...
    methods_cfg = [
        ('TD',   lambda g, t, p, k, s: _run_snapshot_invasive_heuristic(
            g, t, p, k, td_method_adaptive, s, batch_mode=True)),
        ('TIA',  lambda g, t, p, k, s: _run_snapshot_invasive_heuristic(
            g, t, p, k, extended_tia_method_adaptive, s, batch_mode=True)),
        ('Katz', lambda g, t, p, k, s: _run_snapshot_invasive_heuristic(
            g, t, p, k, katz_method_adaptive, s, batch_mode=False)),
    ]
    if use_rl and rl_agent is not None:
        methods_cfg.append((
            'GNN_RL',
            lambda g, t, p, k, s: _run_snapshot_invasive_rl(g, t, rl_agent, p, k, s)
        ))

    os.makedirs(save_dir, exist_ok=True)
    for method_name, run_fn in methods_cfg:
        pc, anc, invasion, success, removed, final_tgts = run_fn(
...
if __name__ == "__main__":
    ...
    DATA_FILE = os.path.join(SCRIPT_DIR, "data", "fig6_data", "invasive_network.graphml")

    MODEL_PATH = os.path.join(REPO_ROOT, "train",
                              "gnn_ppo_dual_stream_n50-100__seed42__1769653775",
                              "model.pt")

    SAVE_DIR = os.path.join(SCRIPT_DIR, "data", "fig6_result", "invasive_species")

    TARGET_RATIO = 0.05
    MAX_PERIPHERAL_DEGREE = 20
    ...
    print("Building Aviation Network for Invasive Species...")
    G, target_nodes = build_invasive_network(
        data_file=DATA_FILE,
    ...
    save_network_with_targets(
        ...
        save_dir=SAVE_DIR,
        graphml_name='invasive_network.graphml',
        txt_name='invasive_target_nodes.txt',
    )

    results = run_invasive_experiment(
        G=G,
        target_nodes=target_nodes,
    ...
    print("\n Invasive Species Spread Experiment Completed!")