...
def load_transit_network(file_path: str) -> Optional[nx.Graph]:
    print(f"Loading transit network from: {os.path.basename(file_path)}")
...
def build_fugitive_network(
    ...
    print("=" * 60)
    print("Building Transit Network for Fugitive Chase Simulation")
    print("=" * 60)

    print("\n1. Loading transit network data...")
    if not os.path.exists(data_file):
        print(f"   Error: File not found: {data_file}")
        return None, None

    G = load_transit_network(data_file)
    if G is None:
        return None, None
    ...
    if not nx.is_connected(G):
        ...
        print(f"   LCC size: {G.number_of_nodes()} stations, {G.number_of_edges()} connections")
    else:
        print(f"   Network is connected ({G.number_of_nodes()} stations)")
    ...
    print(f"\n4. Target count: {n_targets} ({target_ratio:.1%} of {n_total} total stations)")

    print(f"\n5. Identifying peripheral stations (degree ≤ {max_peripheral_degree})...")
    degrees = dict(G.degree())

    peripheral_nodes = [n for n, d in degrees.items() if d <= max_peripheral_degree]
    print(f"   Found {len(peripheral_nodes)} peripheral stations "
    ...
    if len(peripheral_nodes) < n_targets:
        ...
        print(f"   Now have {len(peripheral_nodes)} candidate stations")
    ...
    print(f"\n6. Selecting initial fugitive positions from peripheral stations...")
    ...
    print(f"   Selected {len(target_nodes)} initial fugitives")
    print(f"   Average degree of starting stations: {avg_target_degree:.2f}")

    all_degrees = list(degrees.values())
    hub_nodes = [n for n, d in degrees.items() if d >= 10]

    print(f"\n{'=' * 60}")
    print(f"Rome Transit Network Summary:")
    ...
    print(f"  Peripheral (degree ≤ {max_peripheral_degree}): "
          f"{len([d for d in all_degrees if d <= max_peripheral_degree])} "
          f"({len([d for d in all_degrees if d <= max_peripheral_degree]) / n_total:.1%})")
    print(f"  Hub stations (degree ≥ 10): {len(hub_nodes)} ({len(hub_nodes) / n_total:.1%})")
    print(f"  Initial fugitives: {len(target_nodes)} ({len(target_nodes) / n_total:.2%})")
    print(f"{'=' * 60}")

    return G, target_nodes


class FugitiveSimulator:
    ...
    def _execute_fugitive_movement(self):
        if self.move_prob <= 0:
            return

        current_fugitives = list(self.target_nodes)
        self.rng.shuffle(current_fugitives)

        new_positions = set()
        claimed_positions = set()

        for fugitive_node in current_fugitives:
            if fugitive_node not in self.current_graph:
                continue

            neighbors = list(self.current_graph.neighbors(fugitive_node))
            valid_neighbors = []
            ...
            if valid_neighbors and self.rng.random() < self.move_prob:
                ...
                new_positions.add(fugitive_node)
                claimed_positions.add(fugitive_node)

        self.target_nodes = new_positions
    ...
    def execute_spread_if_round_complete(self):
        if self.attacks_in_current_round >= self.attack_per_step:
            if not self.is_done():
                self._execute_fugitive_movement()
            self.attacks_in_current_round = 0
    ...
    def get_metrics(self) -> Tuple[float, float, float]:
        ...
        pursuit_ratio = len(self.visited_nodes) / max(1, self.original_total_nodes)
        return pc, anc, pursuit_ratio


def simulate_fugitive_heuristic(G: nx.Graph, target_nodes: List[int], move_prob: float,
                                attack_per_step: int, method_func: Callable,
                                seed: Optional[int] = None,
                                batch_mode: bool = False) -> Tuple[float, float, float, bool]:
    simulator = FugitiveSimulator(G, target_nodes, move_prob, attack_per_step, seed)
    ...
    pc, anc, pursuit_ratio = simulator.get_metrics()
    return pc, anc, pursuit_ratio, simulator.is_done()
...
def create_fugitive_pyg_data(G: nx.Graph, target_nodes: set, removed_nodes: set,
...
def simulate_fugitive_rl(G: nx.Graph, target_nodes: List[int], rl_agent,
    ...
    for _ in range(max_steps):
        ...
        data, nodes = create_fugitive_pyg_data(
...
def _run_td_task(args):
    ...
    pc, anc, pursuit, success = simulate_fugitive_heuristic(
        G, targets, move_prob, attack_per_step,
        td_method_adaptive, sim_seed, batch_mode=True
    )
    return {'pc': pc, 'anc': anc, 'pursuit': pursuit,
...
def _run_rl_task(args):
    ...
    if _worker_rl_agent is None:
        return {'pc': None, 'anc': None, 'pursuit': None, 'lambda_eff': lambda_eff, 'sim_seed': sim_seed}

    G = _worker_graph.copy()
    targets = list(_worker_targets)

    pc, anc, pursuit, success = simulate_fugitive_rl(
        G, targets, _worker_rl_agent, move_prob, attack_per_step, sim_seed
    )
    return {'pc': pc, 'anc': anc, 'pursuit': pursuit, 'lambda_eff': lambda_eff, 'sim_seed': sim_seed}


def run_fugitive_experiment(
        G: nx.Graph,
    ...
    print(f"\n{'=' * 70}")
    print(f" Fugitive Chase (Rome Transit): Full 4-Method Comparison")
    print(f"{'=' * 70}")
    print(f"Network: N={G.number_of_nodes()}, E={G.number_of_edges()}")
    print(f"Model: Recoverable Random Walk (stations recover after fugitive departs)")
    print(f"Initial fugitives: {len(target_nodes)}")
    ...
    results = {m: defaultdict(lambda: {'pc': [], 'anc': [], 'pursuit': [], 'success': [], 'seeds': []}) for m in all_methods}
    ...
    with ProcessPoolExecutor(
        ...
        for result in tqdm(executor.map(_run_td_task, td_tasks),
            ...
            results['TD'][result['lambda_eff']]['pursuit'].append(result['pursuit'])
    ...
    with ProcessPoolExecutor(
        ...
        for result in tqdm(executor.map(_run_tia_task, tia_tasks),
            ...
            results['TIA'][result['lambda_eff']]['pursuit'].append(result['pursuit'])
    ...
    with ProcessPoolExecutor(
        ...
        for result in tqdm(executor.map(_run_katz_task, katz_tasks),
            ...
            results['Katz'][result['lambda_eff']]['pursuit'].append(result['pursuit'])
        ...
        with ProcessPoolExecutor(
            ...
            for result in tqdm(executor.map(_run_rl_task, rl_tasks),
                ...
                if result['pc'] is not None:
                    ...
                    results['GNN-RL'][result['lambda_eff']]['pursuit'].append(result['pursuit'])
...
def save_network_with_targets(
        ...
        target_nodes: List[int],
        save_dir:     str,
        graphml_name: str = 'fugitive_network.graphml',
        txt_name:     str = 'fugitive_target_nodes.txt',
) -> None:
    ...
    with open(txt_path, 'w') as fh:
        fh.write('# Fugitive chase (Rome transit) -- initial fugitive positions\n')
        fh.write(
...
def save_results_to_csv(
    ...
    for method, lam_dict in results.items():
        for lam in sorted(lambda_values):
            ...
            qm, qs, qe  = _s(pur_vals) if pur_vals else (0., 0., 0.)
            ...
            rows.append({
                ...
                'pursuit_mean':   qm,  'pursuit_std': qs,  'pursuit_sem': qe,
    ...
    csv_path = os.path.join(save_dir, f'fugitive_results_{ts}.csv')
...
def print_results_summary(results: Dict, lambda_values: List[float]):
    ...
    for lambda_eff in lambda_values:
        print(f"\nλ = {lambda_eff}")
        print(f"{'-' * 90}")
        print(f"{'Method':<15} {'PC (mean±std)':<22} {'ANC (mean±std)':<22} {'Pursuit (mean±std)':<22}")
        print(f"{'-' * 90}")
        ...
        for method in results:
            if results[method][lambda_eff]['pc']:
                ...
                pur_mean = np.mean(results[method][lambda_eff]['pursuit'])
                pur_std = np.std(results[method][lambda_eff]['pursuit'])
                print(
                    f"{method:<15} {pc_mean:.4f}±{pc_std:.4f}          {anc_mean:.4f}±{anc_std:.4f}          {pur_mean:.4f}±{pur_std:.4f}")
...
def plot_results(
    ...
    for method in methods:
        ...
        if lambdas:
            ...
            ax.fill_between(lambdas, np.array(means) - np.array(sems),
                            ...
                            alpha=0.15, color=colors.get(method, '#000'))
    ax.set_xlabel('$\\lambda_{eff}$', fontsize=12)
    ax.set_ylabel('ANC (Transit Network Preserved)', fontsize=12)
    ...
    for method in methods:
        lambdas, means, sems = [], [], []
        for lam in lambda_values:
            vals = results[method][lam]['pursuit']
            if vals:
                lambdas.append(lam)
                means.append(np.mean(vals))
                sems.append(np.std(vals) / np.sqrt(len(vals)))
        if lambdas:
            ...
            ax.fill_between(lambdas, np.array(means) - np.array(sems),
                            np.array(means) + np.array(sems),
                            alpha=0.15, color=colors.get(method, '#000'))
    initial_ratio = n_initial_fugitives / n_nodes
    ax.axhline(y=initial_ratio, color='gray', linestyle='--', linewidth=1.5,
               label=f'Initial fugitives ({initial_ratio:.2%})')
    ax.set_xlabel('$\\lambda_{eff}$', fontsize=12)
    ax.set_ylabel('Pursuit Ratio', fontsize=12)
    ax.set_title('Pursuit Footprint (Unique Stations Visited)', fontsize=13, fontweight='bold')
    ax.legend(loc='best', fontsize=10)
    ax.grid(True, alpha=0.3, linestyle='--')

    target_ratio = n_initial_fugitives / n_nodes
    n_methods = len(methods)
    fig.suptitle(
        f'Fugitive Chase (Rome Transit): {n_methods}-Method Comparison | '
        f'N={n_nodes}, Fugitives={n_initial_fugitives} ({target_ratio:.1%}), Recoverable Walk',
    ...
    if save_dir:
        ...
        filename = f'fugitive_chase_results_{timestamp}.png'
...
class FugitiveSnapshotSimulator(FugitiveSimulator):
...
def _run_snapshot_fugitive_heuristic(G, target_nodes, move_prob, attack_per_step,
                                      method_func, seed, batch_mode=False):
    sim = FugitiveSnapshotSimulator(G, target_nodes, move_prob, attack_per_step, seed)
    ...
    pc, anc, pursuit = sim.get_metrics()
    return pc, anc, pursuit, sim.is_done(), sim.get_removed_nodes(), sim.get_final_targets()


def _run_snapshot_fugitive_rl(G, target_nodes, rl_agent, move_prob,
...
def save_terminal_snapshot(
    ...
    methods_cfg = [
        ('TD',   lambda g, t, p, k, s: _run_snapshot_fugitive_heuristic(
            g, t, p, k, td_method_adaptive, s, batch_mode=True)),
        ('TIA',  lambda g, t, p, k, s: _run_snapshot_fugitive_heuristic(
            g, t, p, k, extended_tia_method_adaptive, s, batch_mode=True)),
        ('Katz', lambda g, t, p, k, s: _run_snapshot_fugitive_heuristic(
            g, t, p, k, katz_method_adaptive, s, batch_mode=False)),
    ]
    if use_rl and rl_agent is not None:
        methods_cfg.append((
            'GNN_RL',
            lambda g, t, p, k, s: _run_snapshot_fugitive_rl(g, t, rl_agent, p, k, s)
        ))

    os.makedirs(save_dir, exist_ok=True)
    for method_name, run_fn in methods_cfg:
        pc, anc, pursuit, success, removed, final_tgts = run_fn(
...
if __name__ == "__main__":
    ...
    DATA_FILE = os.path.join(SCRIPT_DIR, "data", "fig6_data", "fugitive_network.graphml")

    MODEL_PATH = os.path.join(REPO_ROOT, "train",
                              "gnn_ppo_dual_stream_n50-100__seed42__1769653775",
                              "model.pt")

    SAVE_DIR = os.path.join(SCRIPT_DIR, "data", "fig6_result", "fugitive_chase")
    ...
    print("Building Transit Network for Fugitive Chase Simulation...")
    G, target_nodes = build_fugitive_network(
        data_file=DATA_FILE,
    ...
    print(f"  - Peripheral degree threshold: ≤ {MAX_PERIPHERAL_DEGREE}")
    print(f"  - Initial fugitives: {len(target_nodes)} "
    ...
    save_network_with_targets(
        ...
        save_dir=SAVE_DIR,
        graphml_name='fugitive_network.graphml',
        txt_name='fugitive_target_nodes.txt',
    )

    results = run_fugitive_experiment(
        G=G,
        target_nodes=target_nodes,
    ...
    print("\n Fugitive Chase Experiment Completed!")