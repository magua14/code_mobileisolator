...
def load_euroroad_network(file_path: str) -> Optional[nx.Graph]:
    print(f"Loading E-road network from: {os.path.basename(file_path)}")
    ...
    try:
        ...
        print(f"   Loaded {G.number_of_nodes()} cities, {G.number_of_edges()} roads")
...
def build_smuggling_network(
        data_file: str,
        target_ratio: float = 0.05,
        max_peripheral_degree: int = 3,
        seed: int = 42
) -> Tuple[Optional[nx.Graph], Optional[List[int]]]:
    print("=" * 60)
    print("Building E-Road Network for Smuggling Interdiction")
    print("=" * 60)

    print("\n1. Loading E-road network data...")
    if not os.path.exists(data_file):
        print(f"   Error: File not found: {data_file}")
        return None, None

    G = load_euroroad_network(data_file)
    if G is None:
        return None, None
    ...
    if not nx.is_connected(G):
        ...
        print(f"   LCC size: {G.number_of_nodes()} cities, {G.number_of_edges()} roads")
    else:
        print(f"   Network is connected ({G.number_of_nodes()} cities)")

    print("\n3. Verifying node labels...")
    nodes = sorted(G.nodes())
    if nodes != list(range(len(nodes))):
        G = nx.convert_node_labels_to_integers(G)
        print(f"   Relabeled to 0-{G.number_of_nodes() - 1}")
    else:
        print(f"   Labels already consecutive: 0-{G.number_of_nodes() - 1}")

    n_total = G.number_of_nodes()
    n_targets = max(1, int(n_total * target_ratio))
    print(f"\n4. Target count: {n_targets} ({target_ratio:.1%} of {n_total} total cities)")

    print(f"\n5. Identifying peripheral border towns (degree ≤ {max_peripheral_degree})...")
    degrees = dict(G.degree())

    peripheral_nodes = [n for n, d in degrees.items() if d <= max_peripheral_degree]
    print(f"   Found {len(peripheral_nodes)} peripheral towns "
          f"({len(peripheral_nodes) / n_total:.1%} of total)")

    if len(peripheral_nodes) < n_targets:
        expanded_threshold = max_peripheral_degree * 2
        print(f"   Warning: Not enough peripheral nodes ({len(peripheral_nodes)} < {n_targets})")
        print(f"   Expanding to degree ≤ {expanded_threshold}")
        peripheral_nodes = [n for n, d in degrees.items() if d <= expanded_threshold]
        print(f"   Now have {len(peripheral_nodes)} candidate towns")

    if len(peripheral_nodes) < n_targets:
        print(f"   Still not enough, using lowest-degree nodes")
        sorted_by_degree = sorted(degrees.items(), key=lambda x: x[1])
        peripheral_nodes = [n for n, d in sorted_by_degree[:max(n_targets, int(n_total * 0.3))]]
        print(f"   Now have {len(peripheral_nodes)} candidate towns")

    print(f"\n6. Selecting initial courier entry points from peripheral towns...")
    rng = np.random.RandomState(seed)

    target_nodes = rng.choice(peripheral_nodes, size=min(n_targets, len(peripheral_nodes)),
                              replace=False).tolist()

    avg_target_degree = np.mean([degrees[n] for n in target_nodes])
    print(f"   Selected {len(target_nodes)} initial couriers")
    print(f"   Average degree of entry towns: {avg_target_degree:.2f}")

    all_degrees = list(degrees.values())
    hub_nodes = [n for n, d in degrees.items() if d >= 5]

    print(f"\n{'=' * 60}")
    print(f"European E-Road Network Summary:")
    print(f"  Cities (nodes): {G.number_of_nodes()}")
    print(f"  Roads (edges): {G.number_of_edges()}")
    print(f"  Avg degree: {2 * G.number_of_edges() / G.number_of_nodes():.2f}")
    print(f"  Degree range: {min(all_degrees)} - {max(all_degrees)}")
    print(f"  Degree median: {np.median(all_degrees):.1f}")
    print(f"  Peripheral towns (degree ≤ {max_peripheral_degree}): "
          f"{len([d for d in all_degrees if d <= max_peripheral_degree])} "
          f"({len([d for d in all_degrees if d <= max_peripheral_degree]) / n_total:.1%})")
    print(f"  Hub cities (degree ≥ 5): {len(hub_nodes)} ({len(hub_nodes) / n_total:.1%})")
    print(f"  Initial couriers: {len(target_nodes)} ({len(target_nodes) / n_total:.2%})")
    print(f"{'=' * 60}")

    return G, target_nodes


class SmugglingSimulator:

    def __init__(self, G: nx.Graph, target_nodes: List[int], move_prob: float,
                 attack_per_step: int, tau: float = 2.0, seed: Optional[int] = None):

        self.original_graph = G.copy()
        self.current_graph = G.copy()
        self.target_nodes = set(target_nodes)
        self.removed_nodes = set()
        self.exposed_nodes = set(target_nodes)
    ...
    def _softmax_neighbor_selection(self, valid_neighbors: List[int]) -> int:
        current_degrees = dict(self.current_graph.degree())
        degs = [current_degrees.get(nb, 1) for nb in valid_neighbors]

        max_deg = max(degs)
        exp_vals = [math.exp((d - max_deg) / self.tau) for d in degs]
        total = sum(exp_vals)
        probs = [e / total for e in exp_vals]

        return self.rng.choice(valid_neighbors, p=probs)

    def _execute_smuggling_movement(self):
        if self.move_prob <= 0:
            return

        current_fronts = list(self.target_nodes)
        self.rng.shuffle(current_fronts)

        new_front_positions = set()
        claimed_positions = set()
        exposed_this_round = set()

        for front_node in current_fronts:
            if front_node not in self.current_graph:
                continue

            neighbors = list(self.current_graph.neighbors(front_node))
            valid_neighbors = []
            ...
            if valid_neighbors and self.rng.random() < self.move_prob:
                new_pos = self._softmax_neighbor_selection(valid_neighbors)

                claimed_positions.add(new_pos)
                new_front_positions.add(new_pos)

                exposed_this_round.add(front_node)
            else:
                new_front_positions.add(front_node)
                claimed_positions.add(front_node)

        self.exposed_nodes |= exposed_this_round
        self.target_nodes = new_front_positions
    ...
    def execute_spread_if_round_complete(self):
        if self.attacks_in_current_round >= self.attack_per_step:
            if not self.is_done():
                self._execute_smuggling_movement()
            self.attacks_in_current_round = 0
    ...
    def get_metrics(self) -> Tuple[float, float, float]:
        ...
        exposure_ratio = len(self.exposed_nodes) / max(1, self.original_total_nodes)
        return pc, anc, exposure_ratio


def simulate_smuggling_heuristic(G: nx.Graph, target_nodes: List[int], move_prob: float,
    ...
    simulator = SmugglingSimulator(G, target_nodes, move_prob, attack_per_step, tau, seed)
    ...
    pc, anc, exposure_ratio = simulator.get_metrics()
    return pc, anc, exposure_ratio, simulator.is_done()
...
def create_smuggling_pyg_data(G: nx.Graph, target_nodes: set, removed_nodes: set,
                              exposed_nodes: set, move_prob: float, attacks_in_round: int,
...
def simulate_smuggling_rl(G: nx.Graph, target_nodes: List[int], rl_agent,
...
def _init_heuristic_worker(graph_nodes, graph_edges, target_nodes, tau):
    global _worker_graph, _worker_targets, _worker_tau
...
def _init_rl_worker(model_path, graph_nodes, graph_edges, target_nodes, tau):
    global _worker_rl_agent, _worker_graph, _worker_targets, _worker_tau
...
def _run_td_task(args):
    ...
    pc, anc, exposure, success = simulate_smuggling_heuristic(
        G, targets, move_prob, attack_per_step,
        td_method_adaptive, _worker_tau, sim_seed, batch_mode=True
    )
    return {'pc': pc, 'anc': anc, 'exposure': exposure,
...
def _run_tia_task(args):
    ...
    pc, anc, exposure, success = simulate_smuggling_heuristic(
        G, targets, move_prob, attack_per_step,
        extended_tia_method_adaptive, _worker_tau, sim_seed, batch_mode=True
    )
    return {'pc': pc, 'anc': anc, 'exposure': exposure,
...
def _run_katz_task(args):
    ...
    pc, anc, exposure, success = simulate_smuggling_heuristic(
        G, targets, move_prob, attack_per_step,
        katz_method_adaptive, _worker_tau, sim_seed, batch_mode=False
    )
    return {'pc': pc, 'anc': anc, 'exposure': exposure,
...
def _run_rl_task(args):
    ...
    if _worker_rl_agent is None:
        return {'pc': None, 'anc': None, 'exposure': None, 'lambda_eff': lambda_eff, 'sim_seed': sim_seed}

    G = _worker_graph.copy()
    targets = list(_worker_targets)

    pc, anc, exposure, success = simulate_smuggling_rl(
        G, targets, _worker_rl_agent, move_prob, attack_per_step, _worker_tau, sim_seed
    )
    return {'pc': pc, 'anc': anc, 'exposure': exposure, 'lambda_eff': lambda_eff, 'sim_seed': sim_seed}


def run_smuggling_experiment(
        G: nx.Graph,
    ...
    print(f"\n{'=' * 70}")
    print(f" Smuggling Interdiction (E-Road): Full 4-Method Comparison")
    print(f"{'=' * 70}")
    print(f"Network: N={G.number_of_nodes()}, E={G.number_of_edges()}")
    print(f"Model: Non-Destructive Softmax Walk (τ={tau})")
    print(f"Initial couriers: {len(target_nodes)}")
    ...
    results = {m: defaultdict(lambda: {'pc': [], 'anc': [], 'exposure': [], 'success': [], 'seeds': []}) for m in all_methods}
    ...
    with ProcessPoolExecutor(
            ...
            initargs=(static_nodes, static_edges, target_nodes, tau)
    ) as executor:
        for result in tqdm(executor.map(_run_td_task, td_tasks),
            ...
            results['TD'][result['lambda_eff']]['exposure'].append(result['exposure'])
    ...
    with ProcessPoolExecutor(
        ...
        for result in tqdm(executor.map(_run_tia_task, tia_tasks),
            ...
            results['TIA'][result['lambda_eff']]['exposure'].append(result['exposure'])
    ...
    with ProcessPoolExecutor(
        ...
        for result in tqdm(executor.map(_run_katz_task, katz_tasks),
            ...
            results['Katz'][result['lambda_eff']]['exposure'].append(result['exposure'])
    ...
    if use_rl and rl_tasks:
        ...
        with ProcessPoolExecutor(
                ...
                initargs=(model_path, static_nodes, static_edges, target_nodes, tau)
        ) as executor:
            for result in tqdm(executor.map(_run_rl_task, rl_tasks),
                ...
                if result['pc'] is not None:
                    ...
                    results['GNN-RL'][result['lambda_eff']]['exposure'].append(result['exposure'])
...
def save_network_with_targets(
        ...
        target_nodes: List[int],
        save_dir:     str,
        graphml_name: str = 'smuggling_network.graphml',
        txt_name:     str = 'smuggling_target_nodes.txt',
) -> None:
    ...
    with open(txt_path, 'w') as fh:
        fh.write('# Smuggling interdiction (E-road) -- initial courier positions\n')
        fh.write(
...
def save_results_to_csv(
    ...
    for method, lam_dict in results.items():
        for lam in sorted(lambda_values):
            ...
            em, es, ee  = _s(exp_vals) if exp_vals else (0., 0., 0.)
            ...
            rows.append({
                ...
                'exposure_mean':   em,  'exposure_std': es,  'exposure_sem': ee,
    ...
    csv_path = os.path.join(save_dir, f'smuggling_results_tau{tau}_{ts}.csv')
...
def print_results_summary(results: Dict, lambda_values: List[float]):
    ...
    for lambda_eff in lambda_values:
        print(f"\nλ = {lambda_eff}")
        print(f"{'-' * 90}")
        print(f"{'Method':<15} {'PC (mean±std)':<22} {'ANC (mean±std)':<22} {'Exposure (mean±std)':<22}")
        print(f"{'-' * 90}")
        ...
        for method in results:
            if results[method][lambda_eff]['pc']:
                ...
                exp_mean = np.mean(results[method][lambda_eff]['exposure'])
                exp_std = np.std(results[method][lambda_eff]['exposure'])
                print(
                    f"{method:<15} {pc_mean:.4f}±{pc_std:.4f}          {anc_mean:.4f}±{anc_std:.4f}          {exp_mean:.4f}±{exp_std:.4f}")
...
def plot_results(
    ...
    ax.set_ylabel('PC (Checkpoint Cost)', fontsize=12)
    ...
    for method in methods:
        ...
        if lambdas:
            ...
            ax.fill_between(lambdas, np.array(means) - np.array(sems),
                            ...
                            alpha=0.15, color=colors.get(method, '#000'))
    ax.set_xlabel('$\\lambda_{eff}$', fontsize=12)
    ax.set_ylabel('ANC (Road Network Preserved)', fontsize=12)
    ...
    for method in methods:
        lambdas, means, sems = [], [], []
        for lam in lambda_values:
            vals = results[method][lam]['exposure']
            if vals:
                lambdas.append(lam)
                means.append(np.mean(vals))
                sems.append(np.std(vals) / np.sqrt(len(vals)))
        if lambdas:
            ...
            ax.fill_between(lambdas, np.array(means) - np.array(sems),
                            np.array(means) + np.array(sems),
                            alpha=0.15, color=colors.get(method, '#000'))
    initial_ratio = n_initial_couriers / n_nodes
    ax.axhline(y=initial_ratio, color='gray', linestyle='--', linewidth=1.5,
               label=f'Initial couriers ({initial_ratio:.2%})')
    ax.set_xlabel('$\\lambda_{eff}$', fontsize=12)
    ax.set_ylabel('Exposure Ratio', fontsize=12)
    ax.set_title('Exposure Footprint (Exposed Safe Houses)', fontsize=13, fontweight='bold')
    ax.legend(loc='best', fontsize=10)
    ax.grid(True, alpha=0.3, linestyle='--')

    target_ratio = n_initial_couriers / n_nodes
    n_methods = len(methods)
    fig.suptitle(
        f'Smuggling Interdiction (E-Road): {n_methods}-Method Comparison | '
        f'N={n_nodes}, Couriers={n_initial_couriers} ({target_ratio:.1%}), '
        f'Softmax τ={tau}',
    ...
    if save_dir:
        ...
        filename = f'smuggling_results_tau{tau}_{timestamp}.png'
...
class SmugglingSnapshotSimulator(SmugglingSimulator):
    def __init__(self, G, target_nodes, move_prob, attack_per_step, tau=2.0, seed=None):
        super().__init__(G, target_nodes, move_prob, attack_per_step, tau, seed)
...
def _run_snapshot_smuggling_heuristic(G, target_nodes, move_prob, attack_per_step,
                                       method_func, tau, seed, batch_mode=False):
    sim = SmugglingSnapshotSimulator(G, target_nodes, move_prob, attack_per_step, tau, seed)
    ...
    pc, anc, exposure = sim.get_metrics()
    return pc, anc, exposure, sim.is_done(), sim.get_removed_nodes(), sim.get_final_targets()


def _run_snapshot_smuggling_rl(G, target_nodes, rl_agent, move_prob,
    ...
    for _ in range(max_steps):
        ...
        data, nodes = create_smuggling_pyg_data(
            ...
            sim.exposed_nodes, move_prob,
            sim.attacks_in_current_round, attack_per_step)
        action_idx = rl_agent.get_action_deterministic(data)
        node = nodes[action_idx]
...
def save_terminal_snapshot(
    ...
    print(f"\n  Snapshot phase: λ={lambda_eff}, τ={tau}, "
    ...
    methods_cfg = [
        ('TD',   lambda g, t, p, k, s: _run_snapshot_smuggling_heuristic(
            g, t, p, k, td_method_adaptive, tau, s, batch_mode=True)),
        ('TIA',  lambda g, t, p, k, s: _run_snapshot_smuggling_heuristic(
            g, t, p, k, extended_tia_method_adaptive, tau, s, batch_mode=True)),
        ('Katz', lambda g, t, p, k, s: _run_snapshot_smuggling_heuristic(
            g, t, p, k, katz_method_adaptive, tau, s, batch_mode=False)),
    ]
    if use_rl and rl_agent is not None:
        methods_cfg.append((
            'GNN_RL',
            lambda g, t, p, k, s: _run_snapshot_smuggling_rl(g, t, rl_agent, p, k, tau, s)
        ))

    os.makedirs(save_dir, exist_ok=True)
    for method_name, run_fn in methods_cfg:
        pc, anc, exposure, success, removed, final_tgts = run_fn(
        ...
        _save_node_list(
            ...
            header=f'removed nodes — method={method_name} λ={lambda_eff} τ={tau} seed={best_seed}')
        _save_node_list(
            final_tgts,
            os.path.join(save_dir, f'final_targets_{method_name}.txt'),
            header=f'final target nodes — method={method_name} λ={lambda_eff} τ={tau} seed={best_seed}')
...
if __name__ == "__main__":
    ...
    DATA_FILE = os.path.join(SCRIPT_DIR, "data", "fig6_data", "smuggling_network.graphml")

    MODEL_PATH = os.path.join(REPO_ROOT, "train",
                              "gnn_ppo_dual_stream_n50-100__seed42__1769653775",
                              "model.pt")

    SAVE_DIR = os.path.join(SCRIPT_DIR, "data", "fig6_result", "smuggling")
    ...
    print("Building E-Road Network for Smuggling Interdiction...")
    G, target_nodes = build_smuggling_network(
        data_file=DATA_FILE,
        target_ratio=TARGET_RATIO,
        max_peripheral_degree=MAX_PERIPHERAL_DEGREE,
        seed=42
    )
    ...
    print(f"  - Roads: {G.number_of_edges()}")
    print(f"  - Softmax temperature τ: {TAU}")
    print(f"  - Initial couriers: {len(target_nodes)} "
          f"({len(target_nodes) / G.number_of_nodes():.2%})")

    save_network_with_targets(
        G=G,
        target_nodes=target_nodes,
        save_dir=SAVE_DIR,
        graphml_name='smuggling_network.graphml',
        txt_name='smuggling_target_nodes.txt',
    )

    results = run_smuggling_experiment(
        G=G,
        target_nodes=target_nodes,
    ...
    print("\n Smuggling Interdiction Experiment Completed!")