...
TAU_VALUES = [0.1, 0.2, 0.5, 1.0, 2.0, 5.0, 10.0, 20.0, 50.0, 100.0, 200.0, 500.0]
...
def _bfs_distances(G: nx.Graph, source: int) -> Dict[int, float]:
    dist = {source: 0}
    q = deque([source])
    while q:
        u = q.popleft()
...
class StartpointBiasedSimulator:
    def __init__(self, G: nx.Graph, target_nodes: List[int],
        ...
        self._dist_from_start: Dict[int, Dict[int, float]] = {}
        for t in target_nodes:
            if t in G:
                self._dist_from_start[t] = _bfs_distances(G, t)

        self._origin: Dict[int, int] = {t: t for t in target_nodes}
    ...
    def _softmax_choice(self, current_pos: int, neighbors: List[int]) -> int:

        origin = self._origin.get(current_pos, current_pos)
        dist_map = self._dist_from_start.get(origin, {})

        scores = [float(dist_map.get(nb, 0)) for nb in neighbors]

        max_s = max(scores) if scores else 0.0
        exp_v = [math.exp((s - max_s) / self.tau) for s in scores]
        total = sum(exp_v)
        if total == 0:
    ...
    def _execute_target_movement(self):
        ...
        for t in [t for t in self.target_nodes if t in self.current_graph]:
            ...
            if valid:
                new_pos = self._softmax_choice(t, valid)
                movements.append((t, new_pos))
                claimed.add(new_pos)

        for old, new in movements:
            self._origin[new] = self._origin.pop(old, old)
...
def simulate_sp_heuristic(G, target_nodes, move_prob, attack_per_step,
                           method_func, tau, seed=None,
                           batch_mode=False) -> Tuple[float, float, bool]:
    sim       = StartpointBiasedSimulator(G, target_nodes, move_prob,
...
def simulate_sp_rl(G, target_nodes, rl_agent, move_prob, attack_per_step,
...
def _run_heuristic_task_v2(args):
    ...
    pc, anc, success = simulate_sp_heuristic(
...
def save_results_to_csv(all_results: Dict, tau_values: List[float],
    ...
    csv_path = os.path.join(save_dir, f'robustness_startpoint_n{N_NODES}.csv')
...
def plot_robustness(all_results: Dict, tau_values: List[float],
    ...
    col_titles  = [
        'Heterogeneous-Random', 'Heterogeneous-Localized',
        'Homogeneous-Random',   'Homogeneous-Localized',
    ]
    ...
    row_axes_pairs = [(pc_axes, 'pc', r'$P_C$'), (anc_axes, 'anc', 'ANC')]

    for row_axes, metric, ylabel in row_axes_pairs:
    ...
    for col_i, title in enumerate(col_titles):
        pc_axes[col_i].set_title(title, fontsize=7, pad=6)

    labels = list('abcdefgh')
    idx = 0
    for row_axes in [pc_axes, anc_axes]:
        for col_i, ax in enumerate(row_axes):
            xoff = -0.22 if col_i == 0 else -0.18
    ...
    handles = [
        plt.Line2D([], [], color=METHOD_COLORS.get(m, '#333'),
                   ...
                   markeredgewidth=0.3, label=m)
        for m in methods
    ]
    ax_leg.legend(
        handles=handles, loc='center', ncol=len(methods),
    ...
    if save_dir:
        os.makedirs(save_dir, exist_ok=True)
        stem = os.path.join(save_dir, f'robustness_startpoint_n{N_NODES}')
...
if __name__ == '__main__':
    print("Robustness Test: Start-Point-Distance-Biased Movement vs Trained GNN-RL")
    print("=" * 70)
    print(f"  Movement rule  : P(j) ∝ exp(d(j,S)/τ)  [S = initial target position]")
    print(f"  τ → ∞          : uniform random walk")
    print(f"  τ → 0          : greedy outward expansion (strict BFS tree)")
